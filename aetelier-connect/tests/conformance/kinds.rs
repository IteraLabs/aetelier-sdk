...
/// everything is read from the registry. Anchors DAT-RT seeding + txy Book
...
/// the profile codec to the documented canonical pair. Anchors txy venue
...
/// OQ-OB-3), and replaying the fixture through the real ChecksumDelta apply
...
fn checksum_validation(desc: &VenueConformance) -> Outcome {
    ...
    // omits one would fail closed (OQ-OB-3), so the fixture must not.
    ...
    for d in &deltas {
        if !d.is_snapshot {
            match d.checksum {
                ...
                None => {
                    return Outcome::Fail(
                        "a book delta carries no checksum (would fail closed, OQ-OB-3)"
                            .into(),
                    );
                }
            }
...
pub const KINDS: &[Kind] = &[
    ...
    Kind {
        ...
        atlas: "DAT-RT seeding, txy SnapshotSource SSoT",
        invariants: &[],
        run: seeding_taxonomy,
    },
    Kind {
        name: "symbol_canonicalization",
        atlas: "txy venue variation (codec), DAT-RT symbol decode",
        invariants: &[],
    ...
    Kind {
        ...
        atlas: "DAT-OB ChecksumDelta, OQ-OB-3 fail-closed",
        invariants: &[],
